#!/usr/bin/env python3
"""Seed the fold-input state slots (+0x180..+0x2db) from vm_enter_context into
the walker, then capture the keystream (store32) and verify against captured
ciphertext.

This is the decisive fold-closure step: the walker's INIT phase (373 steps) is
key-independent, but the fold reads the key-schedule output slots (+0x1ed/+0x18a/
+0x1dd/+0x265/+0x205/+0x245...), which vm_enter_context captures."""
import struct
...
def main():
    ...
    # Start from idle context, then OVERLAY the fold-input state slots with
    # the live vm_enter_context values.
    ...
    # Overlay the state region 0x180..0x2db from live context
    ctx[0x180:0x2e0] = live[0x180:0x2e0]
    ...
    for dest, val in state["store32"][:16]:
        print("  store32 val=%#010x" % val)
    # verify against captured keystream
