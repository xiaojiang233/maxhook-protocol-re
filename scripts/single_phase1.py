...
if uc is None or rax != EXPECT_RAX:
    print(f"[FAIL] PHASE1 rax={rax if rax is None else hex(rax)} (期望 {hex(EXPECT_RAX)})", flush=True)
    sys.exit(1)
print(f"[OK] PHASE1 解压成功 rax={rax:#x}", flush=True)
