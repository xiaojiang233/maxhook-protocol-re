#!/usr/bin/env python3
"""Diagnose and fix the UC_ERR_READ_UNMAPPED at step 76 (handler 0x180a82c64).
Find what address it reads, then map the key/nonce there."""
import struct
...
def main():
    ...
    state = {"last_jmp": None, "invalid_addr": None}
    ...
    def on_invalid(uc_, access, address, size_, value, user):
        state["invalid_addr"] = (access, address, size_)
        return False
    uc.hook_add(UC_HOOK_CODE, on_code)
    uc.hook_add(UC_HOOK_MEM_INVALID, on_invalid)

    # walk from level 1 as before, but record the invalid address at step 76
    handler = 0x1809f4736
    vip = 0x1815631ee
    key = 0xffffffa5
    for step in range(90):
        state["last_jmp"] = None
        state["invalid_addr"] = None
        ...
        try:
            ...
            pass
        if state["invalid_addr"] is not None:
            access, addr, sz = state["invalid_addr"]
            print("step %d: INVALID MEMORY access=%s addr=%#x size=%d (handler %#x)" % (
                step, "READ" if access == 21 else "WRITE", addr, sz, handler))
            # print registers at failure
            regs = {n: uc.reg_read(r) for n, r in REGS.items()}
            print("  registers:", {n: hex(v) for n, v in regs.items()})
            break
        tgt = state["last_jmp"]
        if tgt is None:
