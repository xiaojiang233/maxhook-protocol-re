...
# ---------- PHASE1 后: RDTSC/CPUID 逐点 hook + PEB 补全 ----------
...
print(f"[RDTSC/CPUID 点: {len(rdtsc_pts)}/{len(cpuid_pts)}]", flush=True)
tsc_counter = {'n': 0}
def on_rdtsc(uc, address, size, ud):
    ...
    if tsc_counter['n'] <= 5:
        print(f"  [RDTSC @{address:#x}] -> {env.tsc:#x}", flush=True)
def on_cpuid(uc, address, size, ud):
    ...
    if eax_in == 0:
        ...
        uc.reg_write(UC_X86_REG_EAX, 0x80000008); uc.reg_write(UC_X86_REG_EBX, 0)
...
print(f"[hooks 注册完成]", flush=True)



# ---------- 预填真机 VM 上下文 ----------
# runtime dump 的 .bugland 0xc000-0xe000 区域 (VM 上下文及周边)
rt2 = open(r'E:/Coding/S1mple/target/MaxHook.runtime-unpacked.dll','rb').read()
bug2 = rt2[0x17c3800:0x17c3800+0x157c000]
# 预填 VM 上下文区域 (0xc000-0xe000) 和周边数据表
for rng in [(0xc000, 0xe000)]:
    s, e = rng
    uc.mem_write(BASE + 0x980000 + s, bug2[s:e])
    print(f"[预填] 0x{s:#x}-0x{e:#x} 真机 VM 上下文 ({e-s} 字节)", flush=True)
# 也预填 0xf900-0xfb00 (含 0xf97c 数据表)
uc.mem_write(BASE + 0x980000 + 0xf900, bug2[0xf900:0xfb00])
print(f"[预填] 0xf900-0xfb00 数据表", flush=True)
