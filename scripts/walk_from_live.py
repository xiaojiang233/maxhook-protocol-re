#!/usr/bin/env python3
"""Start the walker from the vm_enter_context (real persistent state), not the
idle dump.  The real encrypt starts from key=0xffffa301, flag=0x41 (persistent),
not key=0xffffffa5, flag=0x69 (idle).

This should fix the INIT divergence and produce the correct transition state."""
import struct
...
def main():
    ...
    # Use vm_enter_context (real persistent state) as the FULL context
    live = (Path(r"E:\Coding\S1mple\target\vm_context_capture2") / "000006_call_1_meta_vm_enter_context.bin").read_bytes()
    ctx = bytearray(live) + bytearray(768 - len(live))
    # But the handler table pointer +0x85 must be correct
    uc.mem_write(CTX_BASE, bytes(ctx))

    vip = int.from_bytes(live[0x6d:0x75], "little")
    key = int.from_bytes(live[0xa:0xe], "little")
    print("starting from live: vip=%#x key=%#x flag=%#x" % (vip, key, live[0x162]))
    ...
    # Start at the dispatcher (0x180a97f70) with the live VIP
    handler = 0x180a97f70
    for step in range(100):
        ...
        tgt = state["last_jmp"]
        if tgt is None:
            print("step %d: no jmp (handler %#x, RIP=%#x)" % (step, handler, uc.reg_read(UC_X86_REG_RIP)))
            break
        if is_stub(tgt):
