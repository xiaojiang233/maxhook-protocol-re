#!/usr/bin/env python3
"""Diagnose the exact termination at walk step 373: what instruction does the
handler execute that isn't a jmp reg (the "no jmp" case)?

This reveals whether it's a direct jmp (to dispatcher), a ret, or a loop."""
import struct
...
def main():
    ...
    state = {"last_jmp": None, "last_insn": None, "insns": []}
    def on_code(uc_, address, size_, user):
        code = uc_.mem_read(address, min(size_, 15))
        insns = list(md.disasm(code, address))
        if insns:
            state["last_insn"] = (address, insns[0].mnemonic, insns[0].op_str)
            state["insns"].append((address, insns[0].mnemonic, insns[0].op_str))
            if insns[0].mnemonic == "jmp" and insns[0].op_str in REGS:
    ...
    for step in range(400):
        ...
        tgt = state["last_jmp"]
        if tgt is None:
            print("step %d: NO jmp reg. Last insn: %#x %s %s" % (
                step, state["last_insn"][0], state["last_insn"][1], state["last_insn"][2]))
            # print the last few insns executed
            print("  last 5 insns executed in this handler:")
            for a, m, o in state["insns"][-5:]:
                print("    %#x: %s %s" % (a, m, o))
            break
        if is_stub(tgt):
