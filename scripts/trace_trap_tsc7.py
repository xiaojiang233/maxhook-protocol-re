...
try:
    uc.emu_start(ENTRY, BASE + 0x1f00246, timeout=10000, count=0)
...
# 只扫描明文区 (前 3MB, VM handler/初始化代码所在) + 原始 .text
r1, c1 = find_patterns(bug_region[:0x300000], BASE + 0x980000)
...
print(f"RDTSC hook 点: {len(rdtsc_pts)}, CPUID hook 点: {len(cpuid_pts)}", flush=True)

tsc_counter = {'n': 0}
def on_rdtsc(uc, address, size, ud):
    if address not in rdtsc_pts: return  # 范围 hook 会误触发, 精确过滤
    tsc_counter['n'] += 1
...
def on_cpuid(uc, address, size, ud):
    ...
    if eax_in == 0:
        ...
        uc.reg_write(UC_X86_REG_ECX, 0)
        uc.reg_write(UC_X86_REG_EDX, 0)
    uc.reg_write(UC_X86_REG_RIP, address + 2)  # 跳过 cpuid

for pt in rdtsc_pts:
    uc.hook_add(UC_HOOK_CODE, on_rdtsc, begin=pt, end=pt+2)
for pt in cpuid_pts:
    uc.hook_add(UC_HOOK_CODE, on_cpuid, begin=pt, end=pt+2)

except UcError as e:
    print(f"[PHASE1 ERR] {e} RIP=0x{uc.reg_read(UC_X86_REG_RIP):x} tsc调用={tsc_counter['n']}", flush=True)
    # 打印最近执行
    if 'trace' in dir():
        print(f"  trace长度: {len(trace)}")
    raise
...
CHUNK = 50_000_000
t0 = time.time()
print("[CHUNK loop start]", flush=True)
try:
    for blk in range(100):
        uc.emu_start(uc.reg_read(UC_X86_REG_RIP), 0, timeout=30000, count=CHUNK)
        print(f"[*] 块 {blk}: RIP=0x{uc.reg_read(UC_X86_REG_RIP):x} tsc调用={tsc_counter['n']} {time.time()-t0:.1f}s")
        if blk > 0 and blk % 5 == 0: pass
except UcError as e:
    print(f"[!] UcError {e} RIP=0x{uc.reg_read(UC_X86_REG_RIP):x} {time.time()-t0:.1f}s tsc调用={tsc_counter['n']}")
...
if CRASH:
    ...
    try:
        rsp = CRASH['rsp']
        print(f"   [rsp] 附近: {uc.mem_read(rsp, 64).hex()}")
    except Exception: pass
    from capstone import Cs, CS_ARCH_X86, CS_MODE_64
    md = Cs(CS_ARCH_X86, CS_MODE_64)
    print("\n=== 崩溃前最后 80 条指令 ===")
    base_dump = bug_region
    for addr in CRASH['last'][-80:]:
        try:
            off = addr - (BASE + 0x980000)
            insn = next(iter(md.disasm(base_dump[off:off+16], addr, count=1)), None)
            if insn: print(f"  {addr:#x}: {insn.mnemonic:8s} {insn.op_str}")
            else: print(f"  {addr:#x}: ?")
        except Exception:
            print(f"  {addr:#x}: ?")
