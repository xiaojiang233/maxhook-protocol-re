...
path = r'E:/Coding/S1mple/target/MaxHook.dll'
...
# Proven loader fixup: all initialized epochs replace packed sentinel 0x0df09e59.
# Apply only fixup bytes stable across two independent runtime epochs.
_boot = bytes(uc.mem_read(0x180980000, 0x157C000))
_r1 = open(r'E:/Coding/S1mple/target/runtime_bugland.bin','rb').read()
_r2 = open(r'E:/Coding/S1mple/target/runtime_bugland2.bin','rb').read()
_stable = bytearray(_boot); _fixups = 0
for _i, (_x, _y, _z) in enumerate(zip(_boot, _r1, _r2)):
    if _x != _y and _y == _z:
        _stable[_i] = _y; _fixups += 1
uc.mem_write(0x180980000, bytes(_stable))
print(f'[*] applied {_fixups} cross-epoch stable fixup bytes')


# ---------- 聚焦追踪 ----------
GARBAGE_ADDR = 0x18098F97C
TRACE_OUT = r'E:/Coding/S1mple/target/original_same_instance_last_trace.json'  # 垃圾值 0x0df09e59 所在
mem_reads = []
def on_mem_read_garbage(uc, access, address, size, value, ud):
    mem_reads.append((uc.reg_read(UC_X86_REG_RIP), address, size, value))
    print(f"  [读垃圾值] RIP={uc.reg_read(UC_X86_REG_RIP):#x} addr={address:#x} size={size} val={value}")
    return True
uc.hook_add(UC_HOOK_MEM_READ, on_mem_read_garbage, begin=GARBAGE_ADDR, end=GARBAGE_ADDR+8)

handler_log = []
def on_handler_code(uc, address, size, ud):
    rsp = uc.reg_read(UC_X86_REG_RSP)
    top = 0
    try: top = rd(rsp)
    except: pass
    handler_log.append((address, rsp, top, uc.reg_read(UC_X86_REG_RAX), uc.reg_read(UC_X86_REG_R13), uc.reg_read(UC_X86_REG_R14), uc.reg_read(UC_X86_REG_R15), uc.reg_read(UC_X86_REG_RBP)))
uc.hook_add(UC_HOOK_CODE, on_handler_code, begin=0x180c1b230, end=0x180c1b2f4)

def on_ret_trap(uc, address, size, ud):
    rsp = uc.reg_read(UC_X86_REG_RSP)
    print(f"  [RET前] RIP={address:#x} [rsp]={rd(rsp):#x} rsp={rsp:#x}")
    for r, n in [(UC_X86_REG_RAX,'rax'),(UC_X86_REG_RBX,'rbx'),(UC_X86_REG_RCX,'rcx'),(UC_X86_REG_RDX,'rdx'),
        ...
        print(f"    {n} = {uc.reg_read(r):#x}")
    # 栈回溯 32 字节
    try: print(f"    [rsp-0x20..rsp+0x20]: {uc.mem_read(rsp-0x20, 0x40).hex()}")
    except: pass
uc.hook_add(UC_HOOK_CODE, on_ret_trap, begin=0x180c1b2f3, end=0x180c1b2f4)
...
def on_any_unmapped(uc, access, address, size, value, ud):
    regs = {n: uc.reg_read(r) for r,n in [(UC_X86_REG_RAX,'rax'),(UC_X86_REG_RBX,'rbx'),(UC_X86_REG_RCX,'rcx'),(UC_X86_REG_RDX,'rdx'),(UC_X86_REG_RSI,'rsi'),(UC_X86_REG_RDI,'rdi'),(UC_X86_REG_RBP,'rbp'),(UC_X86_REG_RSP,'rsp'),(UC_X86_REG_R8,'r8'),(UC_X86_REG_R9,'r9'),(UC_X86_REG_R10,'r10'),(UC_X86_REG_R11,'r11'),(UC_X86_REG_R12,'r12'),(UC_X86_REG_R13,'r13'),(UC_X86_REG_R14,'r14'),(UC_X86_REG_R15,'r15')]}
    print(f'[UNMAPPED] access={access} rip={uc.reg_read(UC_X86_REG_RIP):#x} addr={address:#x} size={size} value={value:#x} regs={regs}', flush=True)
    return False
uc.hook_add(UC_HOOK_MEM_READ_UNMAPPED | UC_HOOK_MEM_WRITE_UNMAPPED, on_any_unmapped)
...
if CRASH:
    ...
    with open(TRACE_OUT, 'w', encoding='utf-8') as _f: _json.dump(CRASH, _f)
    print(f"\n=== handler 执行次数: {len(handler_log)} ===")
    for i in range(max(0,len(handler_log)-40), len(handler_log)):
        a, rsp, top, rax, r13, r14, r15, rbp = handler_log[i]
        print(f"  h[{i}] {a:#x}: rsp={rsp:#x} [rsp]={top:#x} rax={rax:#x} r13={r13:#x} r14={r14:#x} r15={r15:#x} rbp={rbp:#x}")
    print("\n=== 崩溃现场 ===")
