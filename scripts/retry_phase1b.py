# -*- coding: utf-8 -*-
"""
retry_phase1.py — PHASE1 校验 + 自动重试
unicorn 2.1.4 内存初始态不确定 → PHASE1 解压偶发失败(rax!=0x180C0EB27)或卡死。
策略: 每次尝试重建 Uc + 映射 + PHASE1, 校验 rax, 失败重试, 成功进入 PHASE2。
"""
...
def build_and_phase1():
    """构建 Uc + 映射 + PHASE1, 返回 (uc, rax) 或 None(失败)"""
    try:
        ...
        # PHASE1: 解压 (超时 20s)
        uc.emu_start(ENTRY, BASE + 0x1f00246, timeout=20000, count=0)
...
for attempt in range(30):
    try:
        uc, rax = build_and_phase1()
    except Exception:
        uc, rax = None, None
    if uc is not None and rax == EXPECT_RAX:
        print(f"[OK] 尝试 {attempt+1}: PHASE1 解压成功 rax={rax:#x}", flush=True)
        break
    print(f"[retry {attempt+1}] rax={rax if rax is None else hex(rax)}", flush=True)
    uc = None
    time.sleep(0.3)
else:
    print("[FAIL] 30 次尝试 PHASE1 均失败", flush=True)
    sys.exit(1)

# 校验 .boot 压缩流内容 (内存 vs 文件)
mem_boot = uc.mem_read(BASE + 0x1f00000, 0x100)
file_boot = data[0x896600:0x896600+0x100]
same = sum(1 for a, b in zip(mem_boot, file_boot) if a == b)
print(f"[*] .boot 内存 vs 文件: {same}/256 相同", flush=True)
print(f"    内存头: {mem_boot[:16].hex()}", flush=True)
print(f"    文件头: {file_boot[:16].hex()}", flush=True)
# 保存解压区
unpacked = uc.mem_read(BASE + 0x980000, 0x157C000)
print(f"[*] .bugland 解压区 {len(unpacked)} 字节", flush=True)
...
# PEB/TEB
mods = [('MaxHook.dll', BASE, 0x2E40000, BASE + 0x1f00058)]
for key, dll in env.dlls.items():
...
try:
    for blk in range(100):
        ...
        print(f"[*] 块 {blk}: RIP=0x{uc.reg_read(UC_X86_REG_RIP):x} {time.time()-t0:.1f}s", flush=True)
except UcError as e:
    print(f"[!] UcError {e} RIP=0x{uc.reg_read(UC_X86_REG_RIP):x} {time.time()-t0:.1f}s", flush=True)
...
if CRASH:
    ...
    print("\n=== 崩溃前最后 60 条指令 ===", flush=True)
    base_dump = unpacked
    for addr in CRASH['last'][-60:]:
        try:
            off = addr - (BASE + 0x980000)
            if 0 <= off < len(base_dump):
                insn = next(iter(md.disasm(base_dump[off:off+16], addr, count=1)), None)
