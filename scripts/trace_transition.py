#!/usr/bin/env python3
"""Follow the VM→plaintext-key-schedule transition (indirect call at 0x180322bd3)
and continue executing the plaintext key-schedule to produce keystream.

The walker (walk_chain_v2.py) walks VM INIT 373 steps, reaching the indirect
call at 0x180322bd3 (step 373).  We now follow that call into the plaintext
helper, and continue executing (the plaintext key-schedule + keystream gen)."""
import struct
...
def main():
    ...
    # Walk VM INIT (follow jmp reg stubs) until we reach the transition
    handler = 0x1809f4736
    vip = 0x1815631ee
    key = 0xffffffa5
    for step in range(400):
        ...
        tgt = state["last_jmp"]
        if tgt is None:
            # reached the transition (indirect call, not jmp reg)
            print("reached transition at step %d (handler %#x)" % (step, handler))
            break
        if is_stub(tgt):
    ...
    # Now execute CONTINUOUSLY (not per-handler) to follow the call into plaintext
    # key-schedule.  But the emulator hangs... so instead, let's trace the current
    # RIP and registers at the transition point.
    rip = uc.reg_read(UC_X86_REG_RIP)
    print("at transition: RIP=%#x" % rip)
    regs = {n: hex(uc.reg_read(r)) for n, r in REGS.items()}
