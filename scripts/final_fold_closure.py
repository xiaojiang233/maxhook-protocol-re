#!/usr/bin/env python3
"""Final fold closure attempt: use vm_enter_context (512B, post-prologue real
state) as the starting VM context, then walk the key-schedule to produce
keystream, verifying against captured ciphertext.

The vm_enter_context has key=0xffffa301, flag=0x41 (the real post-prologue
state), vs the idle dump's key=0xffffffa5, flag=0x69."""
import struct
...
def main():
    ...
    # Use vm_enter_context (512B, real post-prologue state) as the context
    live_ctx = (Path(r"E:\Coding\S1mple\target\vm_context_capture2") / "000006_call_1_meta_vm_enter_context.bin").read_bytes()
    ctx = bytearray(live_ctx) + bytearray(768 - len(live_ctx))
    uc.mem_write(CTX_BASE, bytes(ctx))

    # read initial VIP + key from the live context
    vip = struct.unpack("<Q", ctx[0x6d:0x75])[0]
    key = struct.unpack("<I", ctx[0xa:0xe])[0]
    flag = ctx[0x162]
    print("live context: VIP=%#x key=%#x flag=%#x" % (vip, key, flag))
    ...
    # Start: we need the entry handler. The live context's VIP points to the
    # current bytecode position. The handler is determined by the current
    # dispatch state.  Since we don't know the exact handler, we start from
    # the dispatcher and let it dispatch.
    # Actually, start executing at the dispatcher 0x180a97f70.
    handler = 0x180a97f70  # dispatcher
    for step in range(400):
        ...
        if is_stub(tgt):
            ...
            print("step %d: tgt %#x not stub" % (step, tgt))
            break

    print("\nstore32 captured:", len(state["store32"]))
