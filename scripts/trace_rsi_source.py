#!/usr/bin/env python3
"""Trace where rsi (the input struct pointer) is set during the VM INIT phase.
At the transition (0x180322bd0: mov rcx,rsi), rsi must point to the key-schedule
input struct.  Find where rsi is assigned during the 373-step INIT."""
import struct
...
def main():
    ...
    state = {"last_jmp": None, "rsi_history": []}
    def on_code(uc_, address, size_, user):
        code = uc_.mem_read(address, min(size_, 15))
        insns = list(md.disasm(code, address))
        if insns:
            insn = insns[0]
            # record when rsi is written (mov rsi, ...)
            if insn.mnemonic in ("mov","lea","pop") and "rsi" in insn.op_str.split(",")[0]:
                rsi_val = uc_.reg_read(UC_X86_REG_RSI)
                state["rsi_history"].append((address, insn.mnemonic, insn.op_str, rsi_val))
    ...
    # report rsi writes (where the input struct pointer is set)
    print("rsi writes during INIT (last 10):")
    for addr, mnem, ops, val in state["rsi_history"][-10:]:
        print("  %#x: %s %s -> rsi=%#x" % (addr, mnem, ops, val))
