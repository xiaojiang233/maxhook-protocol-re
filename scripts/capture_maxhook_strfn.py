...
Usage:
  python capture_maxhook_strfn.py --compile-only
  uv run --with frida -- python capture_maxhook_strfn.py --process javaw.exe \
      --output .\strfn_capture --ack-sensitive-local-capture --duration 90
"""
...
def parse_args():
    ...
    p.add_argument("--script", type=pathlib.Path, default=here / "capture_maxhook_strfn.js")
...
def main():
    ...
    def on_message(message, data):
        with lock:
            ...
            if data is not None:
                blob = bytes(data)
                if payload.get("kind") == "strfn_addrs":
    ...
    for cid, chunks in addr_chunks.items():
        ...
        (out / f"strfn_call{cid}.txt").write_text("\n".join(lines), encoding="utf-8")
    summ = {"schema": "maxhook.strfn.capture/v1", "pid": pid,
