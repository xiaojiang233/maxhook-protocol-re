...
# 合并相邻点成范围 (gap<0x100), 减少 hook 数量
def merge_ranges(points, gap=0x100):
    pts = sorted(points)
    ranges = []
    if not pts: return ranges
    s = e = pts[0]
    for p in pts[1:]:
        if p - e <= gap:
            e = p
    ...
    ranges.append((s, e+2))
    return ranges
r_ranges = merge_ranges(rdtsc_pts)
c_ranges = merge_ranges(cpuid_pts)
print(f"RDTSC 合并范围: {len(r_ranges)}, CPUID 合并范围: {len(c_ranges)}")
for s, e in r_ranges:
    uc.hook_add(UC_HOOK_CODE, on_rdtsc, begin=s, end=e)
for s, e in c_ranges:
    uc.hook_add(UC_HOOK_CODE, on_cpuid, begin=s, end=e)
...
# ---------- 表读取追踪 ----------
tbl_cnt = {'n': 0}
def on_tbl_read(uc, address, size, ud):
    tbl_cnt['n'] += 1
    if tbl_cnt['n'] > 12: return
    ...
    print(f"  [表读 @{address:#x}] r14={r14:#x} [r14]={v:#x} rbp={rbp:#x} rdx={rdx:#x}")
uc.hook_add(UC_HOOK_CODE, on_tbl_read, begin=0x180a26432, end=0x180a26440)
