...
# TEB 补全单独测试
teb = env.teb
uc.mem_write(teb + 0x00, struct.pack('<Q', teb + 0x2000))
uc.mem_write(teb + 0x08, struct.pack('<Q', 0))
uc.mem_write(teb + 0x20, struct.pack('<Q', RSP))
uc.mem_write(teb + 0x10, struct.pack('<Q', RSP - 0x800000))
uc.mem_write(teb + 0x48, struct.pack('<I', 0x1234))
uc.mem_write(teb + 0x50, struct.pack('<I', 0x5678))
uc.mem_write(teb + 0x60, struct.pack('<Q', teb + 0x3000))
uc.mem_write(teb + 0x680, struct.pack('<I', 0xffffffff))
print("[TEB patched]", flush=True)
