...
crash_state = {}
last_insns = []
def on_crash_trace(uc, address, size, ud):
    last_insns.append(address)
    if len(last_insns) > 5000: last_insns.pop(0)
def on_fetch_unmapped2(uc, access, address, size, value, ud):
    ...
    crash_state['last'] = list(last_insns[-25:])
    return False
uc.hook_add(UC_HOOK_CODE, on_crash_trace)
uc.hook_add(UC_HOOK_MEM_FETCH_UNMAPPED, on_fetch_unmapped2)
# ---------- 第二阶段: WinEnv 模拟 ----------
env.log = print
env.ticks = 0
t0 = time.time()
try:
    uc.emu_start(rax, 0xFFFFFFFFFFFFFFFF, timeout=120000, count=0)
    print(f"[*] 第二阶段返回 RIP=0x{uc.reg_read(UC_X86_REG_RIP):x} {time.time()-t0:.1f}s")
except UcError as e:
    print(f"[!] 第二阶段 UcError {e} RIP=0x{uc.reg_read(UC_X86_REG_RIP):x} {time.time()-t0:.1f}s")
