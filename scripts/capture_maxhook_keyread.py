...
Usage:
  python capture_maxhook_keyread.py --compile-only
  uv run --with frida -- python capture_maxhook_keyread.py --process javaw.exe \
      --output .\keyread_capture --ack-sensitive-local-capture --duration 90
"""
...
def parse_args():
    ...
    p.add_argument("--script", type=pathlib.Path, default=here / "capture_maxhook_keyread.js")
...
def main():
    ...
    def on_message(message, data):
        with lock:
            ...
            if data is not None:
                blob = bytes(data)
                if payload.get("kind") == "keyread_addrs":
    ...
    for cid, chunks in addr_chunks.items():
        ...
        (out / f"keyread_call{cid}.txt").write_text("\n".join(lines), encoding="utf-8")
    summ = {"schema": "maxhook.keyread.capture/v1", "pid": pid,
