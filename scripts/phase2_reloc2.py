...
# 扫描真机上下文里的高地址堆指针 (>= 0x20000000000), 重定位到模拟堆
reloc_map = {}
def reloc_data(region):
    data2 = bytearray(region)
    for i in range(0, len(region) - 8, 8):
        v = struct.unpack('<Q', region[i:i+8])[0]
        if v >= 0x20000000000 and v < 0x7FF000000000:  # 真机堆地址
            if v not in reloc_map:
                reloc_map[v] = env.alloc(0x2000)
            data2[i:i+8] = struct.pack('<Q', reloc_map[v])
    return bytes(data2)
print("[堆指针重定位启用]", flush=True)
...
total_pre = 0
for s, e in diffs:
    uc.mem_write(BASE + 0x980000 + s, reloc_data(bug2[s:e]))
    total_pre += e - s
print(f"[预填] 全部差异区域 {len(diffs)} 段共 {total_pre} 字节", flush=True)
uc.mem_write(BASE + 0x980000 + 0xa000, reloc_data(bug2[0xa000:0x11000]))
print(f"[预填] 扩展区域 0xa000-0x11000, 重定位 {len(reloc_map)} 个堆指针", flush=True)
...
print(f"\n=== API 调用数: {len(env.calls)} ===", flush=True)
for c in env.calls[-20:]:
