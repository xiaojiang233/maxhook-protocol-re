...
# ---------- KUSER_SHARED_DATA + TEB 补全 + 解密写入追踪 ----------
# KUSER_SHARED_DATA @ 0x7FFE0000 (64KB, 但常用前 0x400)
KUSD = 0x7FFE0000
env._ensure_map(KUSD, KUSD + 0x4000)
# 填充常用字段:
# +0x000 TickCountLow / +0x004 TickCountMultiplier / +0x008 InterruptTime
# +0x014 SystemTime (LARGE_INTEGER, 1601 纪元 100ns)
# +0x020 TimeZoneBias
import datetime
now = int(time.time() * 10000000) + 116444736000000000
uc.mem_write(KUSD + 0x14, struct.pack('<Q', now))       # SystemTime
uc.mem_write(KUSD + 0x20, struct.pack('<I', -480))       # TimeZoneBias (UTC+8 = -480 min)
uc.mem_write(KUSD + 0x2C, struct.pack('<I', 10000000))   # TickCountMultiplier
uc.mem_write(KUSD + 0x34, struct.pack('<I', 0))          # TickCount
# TEB 补全
teb = env.teb
uc.mem_write(teb + 0x00, struct.pack('<Q', teb + 0x2000))    # NT_TIB.Self
uc.mem_write(teb + 0x08, struct.pack('<Q', 0))                # ExceptionList
uc.mem_write(teb + 0x20, struct.pack('<Q', RSP))              # StackBase
uc.mem_write(teb + 0x10, struct.pack('<Q', RSP - 0x800000))   # StackLimit
uc.mem_write(teb + 0x48, struct.pack('<I', 0x1234))           # ClientId.UniqueProcess
uc.mem_write(teb + 0x50, struct.pack('<I', 0x5678))           # ClientId.UniqueThread
uc.mem_write(teb + 0x60, struct.pack('<Q', teb + 0x3000))     # ProcessEnvironmentBlock(冗余)
uc.mem_write(teb + 0x680, struct.pack('<I', 0xffffffff))      # LastErrorValue
# 解密写入追踪: hook 0xf900-0xfa00 区域的写
writes = []
def on_tbl_write(uc, access, address, size, value, ud):
    if len(writes) < 20:
        writes.append((uc.reg_read(UC_X86_REG_RIP), address, size, value))
        print(f"  [写解密区] RIP={uc.reg_read(UC_X86_REG_RIP):#x} addr={address:#x} size={size} val={value}")
uc.hook_add(UC_HOOK_MEM_WRITE, on_tbl_write, begin=0x18098F900, end=0x18098FA00)
