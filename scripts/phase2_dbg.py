...
print("[S4] ProcessParameters", flush=True)
...
try:
    for blk in range(60):
        ...
        print(f"[*] 块 {blk}: RIP=0x{uc.reg_read(UC_X86_REG_RIP):x} tsc={tsc_counter['n']} {time.time()-t0:.1f}s", flush=True)
except UcError as e:
