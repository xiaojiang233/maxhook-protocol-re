...
for n, v, vs, ra, rs in sections:
    if n in ('.rdata', '.data'):
        for m in re.finditer(rb'[\x20-\x7e]{6,}', data[ra:ra+rs]):
            s = m.group().decode('latin1')
            rva = v + m.start()
            strings[BASE + rva] = s  # 绝对地址
...
def disasm_region(region_data, region_va, region_off_in_file, label, out_path):
    ...
    CH = 0x1000
    nblocks = len(region_data) // CH
    total_insn = 0
    for bi in range(nblocks):
        off = bi * CH
        block = region_data[off:off+CH]
        if entropy(block) >= 6.5:
            continue  # 数据/加密块跳过
        va = region_va + off
        # 块内反汇编
        insns = list(md.disasm(block, va))
        if len(insns) < 3:
            continue  # 不可解码块
        lines.append(f"; ---- 代码块 @ {va:#x} (熵 {entropy(block):.2f}) ----")
        for insn in insns:
            ann = ''
            if insn.mnemonic == 'lea' and 'rip' in insn.op_str:
                try:
                    target = insn.address + insn.size + int(insn.op_str.split('rip + ')[1], 16)
