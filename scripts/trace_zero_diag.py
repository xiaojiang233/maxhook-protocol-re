...
# 分块映射/写入, 避免 unicorn 大块操作触发 Windows access violation
TOTAL = (last_end + 0xFFF) & ~0xFFF
cur = BASE
while cur < BASE + TOTAL:
    uc.mem_map(cur, min(0x400000, BASE + TOTAL - cur))
    cur += 0x400000
# 显式全清零 (unicorn mem_map 初始内容未定义)
for i in range(0, TOTAL, 0x100000):
    uc.mem_write(BASE + i, b'\x00' * min(0x100000, TOTAL - i))
# 再写文件内容
for i in range(0, len(data), 0x100000):
    uc.mem_write(BASE + i, data[i:i+0x100000])
...
# ---------- 第一阶段: 解压 ----------
print("[P1 start]", flush=True)
try:
    uc.emu_start(ENTRY, BASE + 0x1f00246, timeout=30000, count=0)
    print("[P1 done]", flush=True)
except UcError as e:
    print(f"[P1 ERR] {e} RIP=0x{uc.reg_read(UC_X86_REG_RIP):x}", flush=True)
    raise
