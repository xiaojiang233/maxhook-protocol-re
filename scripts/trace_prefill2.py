...
print("[PHASE1 done] rax=%#x" % rax, flush=True)
# ---------- 预填真机 VM 上下文 ----------
rt2 = open(r'E:/Coding/S1mple/target/MaxHook.runtime-unpacked.dll','rb').read()
bug2 = rt2[0x17c3800:0x17c3800+0x157c000]
uc.mem_write(BASE + 0x980000 + 0xc000, bug2[0xc000:0xe000])
print(f"[预填] VM 上下文 0xc000-0xe000", flush=True)
uc.mem_write(BASE + 0x980000 + 0xf900, bug2[0xf900:0xfb00])
print(f"[预填] 数据表 0xf900-0xfb00", flush=True)
